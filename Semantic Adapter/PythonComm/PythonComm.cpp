...
void PythonComm::init(unsigned long baudrate){
  ...
  buffer[0] = 0;
	for(int i = 0, map_array = 0; i < NUMBER_DYNAMIX;i++){
		...
		for(int j = 0; j < MAX_SIZE_DYNAMIX; j++){
			if(j == 0 && EEPROM[MAX_BYTES *sizeof(char) + MAX_STRINGS * sizeof(int) + 2 + i*MAX_SIZE_DYNAMIX + j] == 0){
        Serial.println("Saving Dynamix in eeprom!");
				initializeDynamix(vl, i);
				break;
		  }
    ...
    //Only save buffer into Pointer if buffer is "full"
    if(buffer[0] != 0){
      va_arg(vl, String) = buffer;
    } else {
      //Get rid of varargs pointer
      va_arg(vl, String);
    }
